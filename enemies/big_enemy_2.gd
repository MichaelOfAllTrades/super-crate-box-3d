...
var x_speed: float = GameConfig.big_enemy_speed_slow
...
func _ready():
	if mesh_instance and mesh_instance.mesh is BoxMesh and collision_shape and collision_shape.shape is BoxShape3D:
		...
		var new_size = Vector3(GameConfig.big_enemy_width, GameConfig.big_enemy_height, GameConfig.big_enemy_width)
	...
	#print("small enemy 2 speed slow", GameConfig.big_enemy_speed_slow)
...
func _physics_process(delta):
	...
	#print(self.enemy_label, " x speed ", GameConfig.big_enemy_speed_slow)
	...
	if self.transform.origin.y < -1 * ((GameConfig.ORIGINAL_HEIGHT * GameConfig.SCALE_FACTOR) / 2):
		...
		if !fell:
			fell = true
			# change enemy speed to GameConfig.big_enemy_speed_fast
			x_speed = GameConfig.big_enemy_speed_fast
	...
	MinimapEvents.emit_signal("entity_position_updated", self, "big enemy " + str(fell), current_pos_2d)

func handle_player_collision():
	queue_free() # here the enemy dies, but in real game PLAYER should die

func take_damage(damage):
	print("big enemy just took ", damage, " damage")
	health -= damage
	if health <= 10:
